...
func TestSearchRecipeWhenSuccessful(t *testing.T) {
	...
	mockedRecipeRepository.On("FindByName", name).Return(recipe, nil)

	router := mux.NewRouter()
	router.HandleFunc("/v1/recipe", SearchRecipeHandler(mockedRecipeRepository)).Methods("GET")
	ts := httptest.NewServer(router)
	defer ts.Close()

	r, err := http.NewRequest("GET", fmt.Sprintf("%s/v1/recipe?name=%s", ts.URL, name), nil)
